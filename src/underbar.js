(function() {
  ...
  _.each = function(collection, iterator) {
    ...
    if(Array.isArray(collection)){   // If Collection is Array
      ...
      for(key in collection){    // If Collection is Object
  ...
  _.uniq = function(array) {
    ...
    _.each(array, function(item){
      if(_.indexOf(returnArr, item) === -1) returnArr.push(item);
    });
  ...
  _.every = function(collection, iterator) {
    ...
    var truth = _.reduce(collection, function(total, current){
      if(iterator === undefined){ // special case if iterator is undefined
        if(current) return total;
        else return total - 1;
      } else {
        if(!iterator(current)){ // if there is a false, subtract
        return total - 1;
        } else return total;  // else dont touch 
      }
    }, 0);
  ...
  _.extend = function(obj) {
    var extendArray = Array.prototype.slice.call(arguments, 1);

    _.each(extendArray, function(x){
      for(var key in x){
        obj[key] = x[key];
      }
    });
  ...
  _.defaults = function(obj) {
    ...
    _.each(extendArray, function(x){
      for(var key in x){
        // Extend IFF NOT DEFINED
        if(obj[key] === undefined) obj[key] = x[key];
      }
    });
    return obj;
  };
